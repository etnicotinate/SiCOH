...
def main():
    ...
    born_vec = np.loadtxt('born.out')
    stre = np.loadtxt('vir.out')    #  virial stress tensor

    try:
    # ave one for fix ave/time
        # born_vec = np.mean(born_vec[:, 1:], axis=0)
    # ave running for fix ave/time
        born_vec = born_vec[-1, 1:]
    except IndexError:
        born_vec = born_vec[1:]
    
    # born: vecor to matrix, stre: remove column 1 (time step)
    born = reduce_Born(born_vec)
    stre = -stre[:, 1:]*101325  # atm->Pa

    # LAMMPS order to Voigt notation
    born[:,[3,5]] = born[:,[5,3]]
    born[[3,5],:] = born[[5,3],:]
    stre[3,5] = stre[5,3]
