...
def plot_QTD(filename):
    ...
    for c in labels:
        plt.plot(T, QTD_counts[c], linewidth=1.5)  # counts
    ...
    plt.savefig('QTD.png', dpi=300, transparent=True)
...
def plot_bonds_perc(filename):
    ...
    for i, bond in enumerate(Bond_types):
        ...
        if bond not in BOND_SELECTED:
            continue
        # plt.figure(figsize=(8,5))    
        plt.plot(T, Percs[i,:], linewidth=1.0 ,marker='o',markersize=1.5)
        labels.append(bond)
