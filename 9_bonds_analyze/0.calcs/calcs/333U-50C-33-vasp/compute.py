...
def plot_curve(mode='pos', dir: str|int='all'):
    ...
    if dir == 'all':
        ...
        for i in [3]*6:
            ...
            plt.plot(eps, sig[:,i-1], linewidth=1.5,marker='o',markersize=3)
            # plt.plot(eps, sig, linewidth=1.5,marker='o',markersize=3)  # display all direction components
    ...
    # smooth the curve
    window, poly = 30, 3
    sig_smooth = savgol_filter(sig[:,dir-1], window, poly)  # window size 11, polynomial order 3
    plt.scatter(eps, sig[:,dir-1], s=3)
    plt.plot(eps, sig_smooth, linewidth=2.0, c='orange')
...
if __name__ == '__main__':
    ...
    # plot_curve(mode='pos',dir='all')
    plot_curve(mode='pos',dir='3')
    # C = read_C(r'C.log')
