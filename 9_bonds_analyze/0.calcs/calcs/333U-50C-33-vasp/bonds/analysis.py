...
BOND_SELECTED = {'Si-O', 'Si-C', 'O-H', 'H-C'}
...
def plot_QTD(filename):
    ...
    strain = .6
    timesteps = np.array([f['timestep'] for f in frames])
    xvalues = timesteps/timesteps[-1]*strain # X axis data: strain

    QTD, QTD_counts = count_QTD_SiO(frames)
    labels = QTD_counts.keys()
    plt.figure(figsize=(15,10))

    print('Initial QTDM counts before deformation: ', end='')
    for c in labels:
        print(f'{c}: {QTD_counts[c][0]}, ', end='')
        plt.scatter(xvalues, QTD_counts[c], s=2.5)  # counts
        # plt.plot(xvalues, QTD_counts[c], linewidth=2.0,marker='o',markersize=2.5)  # counts
    print()
...
def pie_QTD(filename):
    ...
    init_count = [len(v) for v in QTD[0].values()]  # initial frame
    final_count = [len(v) for v in QTD[-1].values()]    # final frame

    plt.figure(figsize=(12,7.5))
    plt.subplot(121)
    # pie chart before deformation
    plt.pie(init_count, autopct='%.2f%%', pctdistance=0.8, labeldistance=1.05, labels=labels)
    plt.title(f'Before deformation',x=0.5,y=1.0)
    # pie chart after deformation
    plt.subplot(122)
    plt.pie(final_count, autopct='%.2f%%', pctdistance=0.8, labeldistance=1.05, labels=labels)
    plt.title(f'After deformation',x=0.5,y=1.0)
    # plt.legend(labels=labels)

    # save PNG
    pngpath = f'QTD_pie.png'
    plt.savefig(pngpath, dpi=300, transparent=True)
    print(f"Pie chart of QTD during deformation has been plotted to {pngpath}")
    return


def plot_bonds(mode='pos', dir: str|int='3'):
    '''
    ...
    for i, bond in enumerate(Bond_types):
        ...
        # Nbonds[Nbonds[i,0] == 0] = 1
        # ref = Nbonds[i,0]
        plt.scatter(xvalues, Nbonds[i,:]/Nbonds[i,0], s=2)
        labels.append(bond)

    plt.xlabel(r'Strain, $\epsilon$')
    plt.ylabel(r'Relative number of bonds')
    ...
    # pie chart
    bond_index=np.array([True if bond in BOND_SELECTED else False for bond in Bond_types ])
    plt.figure(figsize=(12,7.5))
    plt.subplot(121)
    # pie chart before deformation
    plt.pie(Nbonds[bond_index,0], autopct='%.2f%%', pctdistance=0.8, labeldistance=1.05, labels=labels)
    # plt.legend(labels=labels)
    plt.title(f'Pie chart of bond distribution during deformation in {mode}{dir} direction',x=1.0,y=1.0)
    # pie chart after deformation
    plt.subplot(122)
    plt.pie(Nbonds[bond_index,-1], autopct='%.2f%%', pctdistance=0.8, labeldistance=1.05, labels=labels)
    # plt.legend(labels=labels)

    # save PNG
    pngpath = f'bonds.pie_{mode}{dir}.png'
    plt.savefig(pngpath, dpi=300, transparent=True)
    print(f"Pie chart of bond distribution for {mode}{dir} direction has been plotted to {pngpath}\n")
    return

def plot_bonds_abs(mode='pos', dir: str|int='all'):
    '''
    Plot the number of bonds for each bond type in all frames.
