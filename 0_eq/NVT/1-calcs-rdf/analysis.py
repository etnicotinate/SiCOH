...
def print_QTD(filename):
    ...
    # print(f'{QTD_counts=}')
    print(QTD_counts['Q'][0], end='\t')
    print(QTD_counts['T'][0], end='\t')
    print(QTD_counts['D'][0], end='\t')
    print(QTD_counts['M'][0], end='\t')
    print()
...
def pie_QTD(filename):
    ...
    plt.title(f'Substructure distribution before deformation',x=0.5,y=1.0)
    ...
    print(f"Pie chart of substructure distribution for deformation in Z direction has been plotted to {pngpath}\n")
    return
...
def plot_bonds(filename):
    ...
    Nbonds, Bond_types = calc_bonds(frames) # Y axis data: bonds counts; labels: bond types
    ...
    for i, bond in enumerate(Bond_types):
        ...
        # plt.plot(T, norm_Nbonds, linestyle='none', marker='o', markersize=2)
        labels.append(bond)

    # plt.yticks(np.arange(0, 2, 0.2)) # 0-2, step 0.2
    plt.ylabel(r'Relative counts of bonds')
    plt.title(r'Bond statistics during equilibration', x=0.5,y=1.0)
    plt.legend(labels=labels)
    ...
    # plt.legend(labels=labels)
    plt.title(f'Pie chart of bond distribution during equilibration',x=1.0,y=1.0)
    # pie chart after deformation
    plt.subplot(122)
    ...
    # save PNG
    pngpath = f'bonds.pie.png'
    plt.savefig(pngpath, dpi=300, transparent=True)
    print(f"Pie chart of bond distribution for equilibration has been plotted to {pngpath}\n")
...
if __name__ == '__main__':
    # plot_bonds(r'./bonds.reaxff')

    print_QTD(r'./bonds.reaxff')
    # pie_QTD(r'./bonds.reaxff')
